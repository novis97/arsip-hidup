...
export default buildConfig({
  admin: { user: Users.slug, meta: { titleSuffix: ' — Arsip Hidup Indonesia' } },
  ...
  collections: [
    Users, Collections, ArchiveItems, Assets, Transcripts, Narasumber,
    Stories, BatikBusinesses, Themes, Locations, TimelineEvents,
    ColorMapEntries, AccessRequests, AccessGrants, AuditLogs,
  ],
    ...
    s3Storage({
      collections: {
        assets: {
          ...
          generateFileURL: ({ filename }) => `${process.env.PUBLIC_CDN_URL}/${filename}`,
        },
      },
      config: {
