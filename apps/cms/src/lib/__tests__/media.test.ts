...
beforeAll(() => {
  process.env.PAYLOAD_SECRET = "uji-rahasia-yang-cukup-panjang-untuk-hmac";
});
...
describe("token pemutaran", () => {
  ...
  it("menolak payload yang diubah walau tanda tangan lama disertakan", () => {
    ...
    const jahat = Buffer.from(
      JSON.stringify({ ...claims, assetId: "aset-lain", exp: 9e9 }),
    ).toString("base64url");
  ...
  it("menolak token kedaluwarsa", () => {
    ...
    const decoded = JSON.parse(
      Buffer.from(t.split(".")[0], "base64url").toString(),
    );
...
describe("penulisan ulang manifest HLS", () => {
  ...
  it("mengalihkan segmen ke endpoint kita, bukan ke R2 langsung", () => {
    const out = rewriteManifest(m, {
      assetId: "a1",
      token: "tok",
      baseUrl: "https://api.arsiphidup.id",
    });
  ...
  it("ikut mengalihkan URI kunci enkripsi", () => {
    ...
    expect(out).toMatch(
      /#EXT-X-KEY.*URI="https:\/\/api\.arsiphidup\.id\/api\/media\/a1\/segment/,
    );
  });
...
describe("safeSegmentKey — pencegahan path traversal", () => {
  ...
  it("menolak traversal", () =>
    expect(safeSegmentKey(prefix, "../../rahasia.mp4")).toBeNull());
  it("menolak path absolut", () =>
    expect(safeSegmentKey(prefix, "/etc/passwd")).toBeNull());
  it("menolak URI absolut", () =>
    expect(safeSegmentKey(prefix, "https://jahat.example/x.ts")).toBeNull());
  it("menerima segmen relatif biasa", () =>
    expect(safeSegmentKey(prefix, "seg-000.ts")).toBe("hls/a1/seg-000.ts"));
});
