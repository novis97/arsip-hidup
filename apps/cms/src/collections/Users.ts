...
export const Users: CollectionConfig = {
  ...
  fields: [
    { name: 'name', type: 'text', required: true },
    {
      ...
      options: ['admin', 'editor', 'archivist', 'reviewer', 'researcher', 'member'],
    },
    { name: 'mfaEnabled', type: 'checkbox', defaultValue: false, admin: { readOnly: true } },
    { name: 'mfaSecret', type: 'text', hidden: true },
    { name: 'institution', type: 'text', admin: { condition: (d) => d.role === 'researcher' } },
  ],
  hooks: {
    beforeLogin: [
      async ({ user }) => {
        if (MFA_REQUIRED_ROLES.includes(user.role) && !user.mfaEnabled) {
          ...
          throw new Error('MFA wajib diaktifkan untuk peran ini sebelum dapat masuk.');
        }
        return user;
      },
    ],
