...
export const ArchiveItems: CollectionConfig = {
  ...
  fields: [
    ...
    { name: 'slug', type: 'text', required: true, unique: true, index: true,
      admin: { description: 'Tidak boleh diubah setelah terbit. Perubahan slug wajib disertai 301.' } },
    ...
    { name: 'summary', type: 'textarea', required: true, maxLength: 600,
      admin: { description: '2–4 kalimat, kalimat pertama = jawaban (BLUF). Ini yang dikutip mesin jawaban.' } },
    { name: 'description', type: 'richText' },

    // ── Video (brief wajib) ──────────────────────────────────────────
    { name: 'videoSource', type: 'select', required: true, defaultValue: 'none',
      options: [
        { label: 'YouTube (hanya untuk tier publik)', value: 'youtube' },
        { label: 'Internal / R2 (tier terbatas & fallback)', value: 'internal' },
    ...
    { name: 'youtubeId', type: 'text',
      ...
      validate: (val: string | null | undefined, { siblingData }: any) => {
        ...
        return YT_ID.test(val) || 'ID YouTube harus tepat 11 karakter [A-Za-z0-9_-].';
      } },
    { name: 'thumbnailUrl', type: 'text', required: true,
      admin: { description: 'WAJIB dari cdn.arsiphidup.id. Hotlink ke i.ytimg.com merusak façade.' },
      validate: (val: string) =>
        !/ytimg\.com|youtube\.com/.test(val || '') ||
        'Thumbnail tidak boleh hotlink ke domain Google (RULES V-5).' },
    { name: 'youtubePrivacy', type: 'select', options: ['public', 'unlisted'], defaultValue: 'public',
      admin: { condition: (d) => d.videoSource === 'youtube' } },
    { name: 'internalAsset', type: 'relationship', relationTo: 'assets',
      admin: { condition: (d) => d.videoSource === 'internal' } },
    { name: 'fallbackMp4', type: 'text',
      admin: { description: 'Mirror publik di R2 untuk panel fallback (RULES V-11).' } },
    ...
    { name: 'language', type: 'select', hasMany: true, required: true,
      options: ['id', 'jv', 'en'], defaultValue: ['id'] },
    { name: 'accessTier', type: 'select', required: true, defaultValue: 'public',
      options: ['public', 'restricted'] },
    { name: 'embargoUntil', type: 'date',
      admin: { description: 'Sebelum tanggal ini hanya metadata yang tampil (RULES E-5).' } },
    { name: 'rightsStatement', type: 'text', required: true },
    { name: 'license', type: 'text', required: true, defaultValue: 'CC BY-NC-ND 4.0' },

    // ── Consent: gerbang publikasi ───────────────────────────────────
    { name: 'consentRef', type: 'text', required: true,
      admin: { description: 'KODE dokumen saja (CNS-2026-014). Dokumen fisiknya TIDAK PERNAH diunggah ke sistem ini.' } },
    { name: 'consentVerified', type: 'checkbox', defaultValue: false },
    { name: 'withdrawalRequested', type: 'checkbox', defaultValue: false,
      admin: { description: 'Dicentang = seluruh item ditarik dari publik dalam ≤24 jam (RULES E-3).' } },

    { name: 'contributors', type: 'array', fields: [
      { name: 'narasumber', type: 'relationship', relationTo: 'narasumber', required: true },
      { name: 'role', type: 'select', options: ['narasumber', 'pewawancara', 'juru-kamera', 'penerjemah'], required: true },
    ] },
  ...
  hooks: {
    beforeValidate: [
      ({ data }) => {
        ...
        if (data.accessTier === 'restricted' && data.videoSource === 'youtube') {
          throw new Error(
        ...
        if (data._status === 'published' && !data.consentVerified) {
          throw new Error('Item tidak dapat diterbitkan sebelum consentVerified dicentang. (RULES §1.2)');
        }
