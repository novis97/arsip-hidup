...
function logAudit(eventType: string, itemId: string, meta: Record<string, unknown> = {}) {
  try {
...
function showFallback(root: HTMLElement) {
  ...
  if (!box) return;
  const actions = box.querySelector<HTMLElement>('.arsip-player__fallback-actions')!;
  ...
  box.hidden = false;
  root.querySelector<HTMLElement>('.arsip-player__frame')?.setAttribute('hidden', '');
}
...
function mountYouTube(root: HTMLElement, videoId: string) {
  ...
  const timer = window.setTimeout(() => showFallback(root), FALLBACK_TIMEOUT_MS);
      ...
      const player = new YT.Player(host, {
        ...
        events: {
          ...
          onStateChange: (e: any) => {
            ...
            if (e.data === S.PLAYING) {
              logAudit('video.play', root.dataset.itemId!, { source: 'youtube', tier: 'public' });
            }
            if (e.data === S.ENDED) {
              // Cegah end-screen YouTube tanpa menutupi elemen player (RULES V-7).
              player.stopVideo();
              showAfterPanel(root);
              logAudit('video.complete', root.dataset.itemId!, { source: 'youtube' });
            }
          },
          onError: () => { clearTimeout(timer); showFallback(root); },
        },
      });
    })
    .catch(() => { clearTimeout(timer); showFallback(root); });
}
...
function wireControls(root: HTMLElement, player: any) {
  ...
  const q = (a: string) => bar.querySelector(`[data-act="${a}"]`) as HTMLElement;
  ...
  root.addEventListener('keydown', (e: KeyboardEvent) => {
    ...
    const map: Record<string, () => void> = {
      ...
      arrowleft: () => player.seekTo(Math.max(0, player.getCurrentTime() - 5), true),
...
function init() {
  document.querySelectorAll<HTMLElement>('[data-arsip-player]').forEach((root) => {
