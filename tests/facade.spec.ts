...
test("nol request ke domain Google sebelum klik Play (RULES V-4)", async ({
  ...
  page.on("request", (r) => {
    if (DOMAIN_GOOGLE.test(r.url())) terlarang.push(r.url());
  });
  ...
  expect(
    terlarang,
    `Façade bocor. Request ke Google sebelum Play:\n${terlarang.join("\n")}`,
  ).toEqual([]);
});

test("thumbnail dilayani dari CDN sendiri, bukan ytimg (RULES V-5)", async ({
  page,
}) => {
  await page.goto(HALAMAN);
  const src = await page
    .locator(".arsip-player__frame img")
    .first()
    .getAttribute("src");
...
test("iframe memakai youtube-nocookie dan sandbox tanpa allow-popups (RULES V-6)", async ({
  page,
}) => {
  await page.goto(HALAMAN);
...
test("transkrip ada di HTML awal, bukan disuntik JavaScript (GEO §1)", async ({
  request,
}) => {
...
test("panel fallback muncul bila YouTube diblokir (RULES V-11)", async ({
  page,
  context,
}) => {
